...
class ViewerPanel(BasePanel):
    ...
    def _build_browse(self, parent):
        from .explorer import HierarchyExplorer
        self.explorer = HierarchyExplorer(
            parent,
            self.conn,
            on_select=self._on_explorer_select,
            on_tool_request=self._on_tool_request,
        )
        self.explorer.pack(fill="both", expand=True, padx=4, pady=4)

        # Keep chunks_data for backward compat with detail panel
        self.chunks_data = []
    ...
    def _load_files(self):
        # Explorer handles its own loading — just refresh if it exists
        if hasattr(self, 'explorer'):
            self.explorer.refresh()

    def _on_explorer_select(self, node_id, chunk_id, file_cid):
        """Called when a node is selected in the HierarchyExplorer."""
        if chunk_id:
            self._show_chunk_detail(chunk_id)
        elif file_cid:
            # Selected a file node — show its chunks in the detail panel
            chunks = self.qmod.get_chunks_for_file(self.conn, file_cid)
            if chunks:
                self._show_chunk_detail(chunks[0]["chunk_id"])

    def _on_tool_request(self, item):
        """Called from the explorer right-click menu to run a tool."""
        # Switch to Curate panel with context
        # For now, just indicate the feature is available
        from tkinter import messagebox
        messagebox.showinfo(
            "Run Tool",
            f"Tool integration point\n\n"
            f"Node: {item.name}\n"
            f"Type: {item.node_type}\n"
            f"Path: {item.path}\n\n"
            f"Switch to the Curate panel to run tools.",
            parent=self.app,
        )
    ...
    def _on_file_select(self, event):
        # Legacy handler — explorer handles selection now
        pass
